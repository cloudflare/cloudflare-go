...
func TestListAccountLoadBalancerPools(t *testing.T) {
	setup()
	defer teardown()
	...
	mux.HandleFunc("/accounts/01a7362d577a6c3019a474fd6f485823/load_balancers/pools", handler)
	createdOn, _ := time.Parse(time.RFC3339, "2014-01-01T05:20:00.12345Z")
	modifiedOn, _ := time.Parse(time.RFC3339, "2014-02-01T05:20:00.12345Z")
	want := []LoadBalancerPool{
		{
	...
	actual, err := client.ListAccountLoadBalancerPools(context.Background(), "01a7362d577a6c3019a474fd6f485823")
	if assert.NoError(t, err) {
		assert.Equal(t, want, actual)
	}
}
