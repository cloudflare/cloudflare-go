...
// table
func (r *DeviceFleetStatusService) Get(ctx context.Context, accountTag string, deviceID string, query DeviceFleetStatusGetParams, opts ...option.RequestOption) (res *DeviceFleetStatusGetResponse, err error) {
	opts = append(r.Options[:], opts...)
	if accountTag == "" {
		err = errors.New("missing required accountTag parameter")
		return
	}
	if deviceID == "" {
	...
	path := fmt.Sprintf("accounts/%s/devices/%s/fleet-status/live", accountTag, deviceID)
