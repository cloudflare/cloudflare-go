...
// WaitingRoomPreviewService contains methods and other services that help with
// interacting with the cloudflare API. Note, unlike clients, this service does not
// read variables from the environment automatically. You should not instantiate
// this service directly, and instead use the [NewWaitingRoomPreviewService] method
// instead.
type WaitingRoomPreviewService struct {
	Options []option.RequestOption
}

// NewWaitingRoomPreviewService generates a new service that applies the given
// options to each request. These options are applied after the parent client's
// options (if there is one), and before any request-specific options.
func NewWaitingRoomPreviewService(opts ...option.RequestOption) (r *WaitingRoomPreviewService) {
	r = &WaitingRoomPreviewService{}
	r.Options = opts
	return
}
...
func (r *WaitingRoomPreviewService) New(ctx context.Context, zoneIdentifier string, body WaitingRoomPreviewNewParams, opts ...option.RequestOption) (res *WaitingRoomPreviewNewResponse, err error) {
	opts = append(r.Options[:], opts...)
	var env WaitingRoomPreviewNewResponseEnvelope
...
type WaitingRoomPreviewNewResponse struct {
	// URL where the custom waiting room page can temporarily be previewed.
	PreviewURL string                            `json:"preview_url"`
	JSON       waitingRoomPreviewNewResponseJSON `json:"-"`
}

// waitingRoomPreviewNewResponseJSON contains the JSON metadata for the struct
// [WaitingRoomPreviewNewResponse]
type waitingRoomPreviewNewResponseJSON struct {
	PreviewURL  apijson.Field
	raw         string
	ExtraFields map[string]apijson.Field
}

func (r *WaitingRoomPreviewNewResponse) UnmarshalJSON(data []byte) (err error) {
	return apijson.UnmarshalRoot(data, r)
}

type WaitingRoomPreviewNewParams struct {
...
func (r WaitingRoomPreviewNewParams) MarshalJSON() (data []byte, err error) {
	return apijson.MarshalRoot(r)
}

type WaitingRoomPreviewNewResponseEnvelope struct {
	Result WaitingRoomPreviewNewResponse             `json:"result,required"`
	JSON   waitingRoomPreviewNewResponseEnvelopeJSON `json:"-"`
}

// waitingRoomPreviewNewResponseEnvelopeJSON contains the JSON metadata for the
// struct [WaitingRoomPreviewNewResponseEnvelope]
type waitingRoomPreviewNewResponseEnvelopeJSON struct {
	Result      apijson.Field
	raw         string
	ExtraFields map[string]apijson.Field
}

func (r *WaitingRoomPreviewNewResponseEnvelope) UnmarshalJSON(data []byte) (err error) {
	return apijson.UnmarshalRoot(data, r)
}
