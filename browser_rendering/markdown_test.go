...
func TestMarkdownNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.BrowserRendering.Markdown.New(context.TODO(), browser_rendering.MarkdownNewParams{
		...
		Body: browser_rendering.MarkdownNewParamsBodyObject{
			...
			AddScriptTag: cloudflare.F([]browser_rendering.MarkdownNewParamsBodyObjectAddScriptTag{{
				ID:      cloudflare.F("id"),
			...
			Authenticate: cloudflare.F(browser_rendering.MarkdownNewParamsBodyObjectAuthenticate{
				Password: cloudflare.F("x"),
				Username: cloudflare.F("x"),
			}),
			BestAttempt: cloudflare.F(true),
			Cookies: cloudflare.F([]browser_rendering.MarkdownNewParamsBodyObjectCookie{{
				...
				Priority:     cloudflare.F(browser_rendering.MarkdownNewParamsBodyObjectCookiesPriorityLow),
				SameParty:    cloudflare.F(true),
				SameSite:     cloudflare.F(browser_rendering.MarkdownNewParamsBodyObjectCookiesSameSiteStrict),
				Secure:       cloudflare.F(true),
				SourcePort:   cloudflare.F(0.000000),
				SourceScheme: cloudflare.F(browser_rendering.MarkdownNewParamsBodyObjectCookiesSourceSchemeUnset),
				URL:          cloudflare.F("url"),
			}}),
			...
			GotoOptions: cloudflare.F(browser_rendering.MarkdownNewParamsBodyObjectGotoOptions{
				...
				WaitUntil:      cloudflare.F[browser_rendering.MarkdownNewParamsBodyObjectGotoOptionsWaitUntilUnion](browser_rendering.MarkdownNewParamsBodyObjectGotoOptionsWaitUntilString(browser_rendering.MarkdownNewParamsBodyObjectGotoOptionsWaitUntilStringLoad)),
			}),
			RejectRequestPattern: cloudflare.F([]string{"string"}),
			RejectResourceTypes:  cloudflare.F([]browser_rendering.MarkdownNewParamsBodyObjectRejectResourceType{browser_rendering.MarkdownNewParamsBodyObjectRejectResourceTypeDocument}),
			SetExtraHTTPHeaders: cloudflare.F(map[string]string{
				"foo": "string",
			}),
			SetJavaScriptEnabled: cloudflare.F(true),
			UserAgent:            cloudflare.F("userAgent"),
			Viewport: cloudflare.F(browser_rendering.MarkdownNewParamsBodyObjectViewport{
			...
			WaitForSelector: cloudflare.F(browser_rendering.MarkdownNewParamsBodyObjectWaitForSelector{
				Selector: cloudflare.F("selector"),
				Hidden:   cloudflare.F(browser_rendering.MarkdownNewParamsBodyObjectWaitForSelectorHiddenTrue),
				Timeout:  cloudflare.F(120000.000000),
				Visible:  cloudflare.F(browser_rendering.MarkdownNewParamsBodyObjectWaitForSelectorVisibleTrue),
			}),
