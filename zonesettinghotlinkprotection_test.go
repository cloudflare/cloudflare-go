...
func TestZoneSettingHotlinkProtectionEdit(t *testing.T) {
	...
	_, err := client.Zones.Settings.HotlinkProtection.Edit(context.TODO(), cloudflare.ZoneSettingHotlinkProtectionEditParams{
		...
		Value:  cloudflare.F(cloudflare.ZoneSettingHotlinkProtectionEditParamsValueOn),
	})
	if err != nil {
...
func TestZoneSettingHotlinkProtectionGet(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
