...
func TestWorkerDomainListWithOptionalParams(t *testing.T) {
...
func TestWorkerDomainReplace(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Workers.Domains.Replace(
		context.TODO(),
		"9a7806061c88ada191ed06f989cc3dac",
		cloudflare.WorkerDomainReplaceParams{
			Environment: cloudflare.F("production"),
			Hostname:    cloudflare.F("foo.example.com"),
			Service:     cloudflare.F("foo"),
			ZoneID:      cloudflare.F[any]("593c9c94de529bbbfaac7c53ced0447d"),
