...
func TestZoneSettingPrefetchPreloadEdit(t *testing.T) {
	...
	_, err := client.Zones.Settings.PrefetchPreload.Edit(context.TODO(), cloudflare.ZoneSettingPrefetchPreloadEditParams{
		...
		Value:  cloudflare.F(cloudflare.ZoneSettingPrefetchPreloadEditParamsValueOn),
	})
	if err != nil {
...
func TestZoneSettingPrefetchPreloadGet(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
