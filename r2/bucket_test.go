...
func TestBucketListWithOptionalParams(t *testing.T) {
	...
	_, err := client.R2.Buckets.List(context.TODO(), r2.BucketListParams{
		...
		CfR2Jurisdiction: cloudflare.F(r2.BucketListParamsCfR2JurisdictionDefault),
	})
	if err != nil {
		var apierr *cloudflare.Error
		if errors.As(err, &apierr) {
...
func TestBucketEditWithOptionalParams(t *testing.T) {
	t.Skip("TODO: investigate broken test")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.R2.Buckets.Edit(
		context.TODO(),
		"example-bucket",
		r2.BucketEditParams{
			AccountID:        cloudflare.F("023e105f4ecef8ad9ca31a8372d0c353"),
			StorageClass:     cloudflare.F(r2.BucketEditParamsCfR2StorageClassStandard),
			CfR2Jurisdiction: cloudflare.F(r2.BucketEditParamsCfR2JurisdictionDefault),
		},
	)
	if err != nil {
		var apierr *cloudflare.Error
