...
func TestZeroTrustDevicePolicyExcludeUpdate(t *testing.T) {
...
func TestZeroTrustDevicePolicyExcludeList(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
...
func TestZeroTrustDevicePolicyExcludeGet(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.ZeroTrust.Devices.Policies.Excludes.Get(
		...
		cloudflare.ZeroTrustDevicePolicyExcludeGetParams{
			AccountID: cloudflare.F[any]("699d98642c564d2e855e9661899b7252"),
		},
	)
	if err != nil {
