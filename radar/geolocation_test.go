...
func TestGeolocationListWithOptionalParams(t *testing.T) {
...
func TestGeolocationGetWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Radar.Geolocations.Get(
		context.TODO(),
		"3190509",
		radar.GeolocationGetParams{
			Format: cloudflare.F(radar.GeolocationGetParamsFormatJson),
		},
	)
	if err != nil {
		var apierr *cloudflare.Error
