...
func TestEntityASNAsSetWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Radar.Entities.ASNs.AsSet(
		context.TODO(),
		int64(3),
		radar.EntityASNAsSetParams{
			Format: cloudflare.F(radar.EntityASNAsSetParamsFormatJson),
		},
	)
	if err != nil {
		var apierr *cloudflare.Error
...
func TestEntityASNBotnetThreatFeedWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Radar.Entities.ASNs.BotnetThreatFeed(context.TODO(), radar.EntityASNBotnetThreatFeedParams{
		ASN:              cloudflare.F([]string{"string"}),
		CompareDateRange: cloudflare.F("7d"),
		Date:             cloudflare.F(time.Now()),
		Format:           cloudflare.F(radar.EntityASNBotnetThreatFeedParamsFormatJson),
		Limit:            cloudflare.F(int64(1)),
		Location:         cloudflare.F("US"),
		Metric:           cloudflare.F(radar.EntityASNBotnetThreatFeedParamsMetricOffenseCount),
		Offset:           cloudflare.F(int64(0)),
		SortOrder:        cloudflare.F(radar.EntityASNBotnetThreatFeedParamsSortOrderAsc),
	})
	if err != nil {
		var apierr *cloudflare.Error
		if errors.As(err, &apierr) {
