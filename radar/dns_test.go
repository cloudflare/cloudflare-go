...
func TestDNSSummaryV2WithOptionalParams(t *testing.T) {
	...
	_, err := client.Radar.DNS.SummaryV2(
		...
		radar.DNSSummaryV2Params{
			...
			Protocol:      cloudflare.F(radar.DNSSummaryV2ParamsProtocolUdp),
			QueryType:     cloudflare.F(radar.DNSSummaryV2ParamsQueryTypeA),
			ResponseCode:  cloudflare.F(radar.DNSSummaryV2ParamsResponseCodeNoerror),
...
func TestDNSTimeseriesWithOptionalParams(t *testing.T) {
	...
	_, err := client.Radar.DNS.Timeseries(context.TODO(), radar.DNSTimeseriesParams{
		...
		Name:         cloudflare.F([]string{"main_series"}),
		Nodata:       cloudflare.F(true),
		Protocol:     cloudflare.F(radar.DNSTimeseriesParamsProtocolUdp),
		QueryType:    cloudflare.F(radar.DNSTimeseriesParamsQueryTypeA),
		ResponseCode: cloudflare.F(radar.DNSTimeseriesParamsResponseCodeNoerror),
		Tld:          cloudflare.F([]string{"string"}),
	})
	if err != nil {
		var apierr *cloudflare.Error
...
func TestDNSTimeseriesGroupsV2WithOptionalParams(t *testing.T) {
	...
	_, err := client.Radar.DNS.TimeseriesGroupsV2(
		...
		radar.DNSTimeseriesGroupsV2Params{
			...
			Name:          cloudflare.F([]string{"main_series"}),
			Nodata:        cloudflare.F(true),
			Protocol:      cloudflare.F(radar.DNSTimeseriesGroupsV2ParamsProtocolUdp),
			QueryType:     cloudflare.F(radar.DNSTimeseriesGroupsV2ParamsQueryTypeA),
			ResponseCode:  cloudflare.F(radar.DNSTimeseriesGroupsV2ParamsResponseCodeNoerror),
			Tld:           cloudflare.F([]string{"string"}),
		},
	)
	if err != nil {
