...
func TestChallengeWidgetReplaceWithOptionalParams(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Challenges.Widgets.Replace(
		context.TODO(),
		"023e105f4ecef8ad9ca31a8372d0c353",
		"0x4AAF00AAAABn0R22HWm-YUc",
		cloudflare.ChallengeWidgetReplaceParams{
			Domains:        cloudflare.F([]string{"203.0.113.1", "cloudflare.com", "blog.example.com"}),
			Mode:           cloudflare.F(cloudflare.ChallengeWidgetReplaceParamsModeInvisible),
			Name:           cloudflare.F("blog.cloudflare.com login form"),
			BotFightMode:   cloudflare.F(true),
			ClearanceLevel: cloudflare.F(cloudflare.ChallengeWidgetReplaceParamsClearanceLevelInteractive),
			Offlabel:       cloudflare.F(true),
		},
	)
	if err != nil {
