...
func (r *ClipService) New(ctx context.Context, params ClipNewParams, opts ...option.RequestOption) (res *ClipNewResponse, err error) {
...
type ClipNewResponse struct {
	...
	Playback ClipNewResponsePlayback `json:"playback"`
	...
	Status ClipNewResponseStatus `json:"status"`
	// The timestamp for a thumbnail image calculated as a percentage value of the
	// video's duration. To convert from a second-wise timestamp to a percentage,
	// divide the desired timestamp by the total duration of the video. If this value
	// is not set, the default thumbnail image is taken from 0s of the video.
	ThumbnailTimestampPct float64                  `json:"thumbnailTimestampPct"`
	Watermark             ClipNewResponseWatermark `json:"watermark"`
	JSON                  clipNewResponseJSON      `json:"-"`
}

// clipNewResponseJSON contains the JSON metadata for the struct [ClipNewResponse]
type clipNewResponseJSON struct {
...
func (r *ClipNewResponse) UnmarshalJSON(data []byte) (err error) {
	return apijson.UnmarshalRoot(data, r)
}

func (r clipNewResponseJSON) RawJSON() string {
	return r.raw
}

type ClipNewResponsePlayback struct {
	// DASH Media Presentation Description for the video.
	Dash string `json:"dash"`
	// The HLS manifest for the video.
	Hls  string                      `json:"hls"`
	JSON clipNewResponsePlaybackJSON `json:"-"`
}

// clipNewResponsePlaybackJSON contains the JSON metadata for the struct
// [ClipNewResponsePlayback]
type clipNewResponsePlaybackJSON struct {
	Dash        apijson.Field
	Hls         apijson.Field
	raw         string
	ExtraFields map[string]apijson.Field
...
func (r *ClipNewResponsePlayback) UnmarshalJSON(data []byte) (err error) {
	return apijson.UnmarshalRoot(data, r)
}

func (r clipNewResponsePlaybackJSON) RawJSON() string {
	return r.raw
}

// Specifies the processing status for all quality levels for a video.
type ClipNewResponseStatus string

const (
	ClipNewResponseStatusPendingupload ClipNewResponseStatus = "pendingupload"
	ClipNewResponseStatusDownloading   ClipNewResponseStatus = "downloading"
	ClipNewResponseStatusQueued        ClipNewResponseStatus = "queued"
	ClipNewResponseStatusInprogress    ClipNewResponseStatus = "inprogress"
	ClipNewResponseStatusReady         ClipNewResponseStatus = "ready"
	ClipNewResponseStatusError         ClipNewResponseStatus = "error"
)

type ClipNewResponseWatermark struct {
	// The unique identifier for the watermark profile.
	Uid  string                       `json:"uid"`
	JSON clipNewResponseWatermarkJSON `json:"-"`
}

// clipNewResponseWatermarkJSON contains the JSON metadata for the struct
// [ClipNewResponseWatermark]
type clipNewResponseWatermarkJSON struct {
	Uid         apijson.Field
	raw         string
	ExtraFields map[string]apijson.Field
}

func (r *ClipNewResponseWatermark) UnmarshalJSON(data []byte) (err error) {
	return apijson.UnmarshalRoot(data, r)
}

func (r clipNewResponseWatermarkJSON) RawJSON() string {
	return r.raw
}
...
type ClipNewResponseEnvelope struct {
	...
	Result   ClipNewResponse                   `json:"result,required"`
