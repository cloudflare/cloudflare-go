...
func TestAccountReplaceWithOptionalParams(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Accounts.Replace(
		context.TODO(),
		map[string]interface{}{},
		cloudflare.AccountReplaceParams{
			Name: cloudflare.F("Demo Account"),
			Settings: cloudflare.F(cloudflare.AccountReplaceParamsSettings{
				DefaultNameservers:          cloudflare.F(cloudflare.AccountReplaceParamsSettingsDefaultNameserversCloudflareStandard),
