...
func TestPageShieldPolicyReplaceWithOptionalParams(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.PageShield.Policies.Replace(
		context.TODO(),
		"023e105f4ecef8ad9ca31a8372d0c353",
		"c9ef84a6bf5e47138c75d95e2f933e8f",
		cloudflare.PageShieldPolicyReplaceParams{
			Action:      cloudflare.F(cloudflare.PageShieldPolicyReplaceParamsActionAllow),
			Description: cloudflare.F("Checkout page CSP policy"),
			Enabled:     cloudflare.F(true),
			Expression:  cloudflare.F("ends_with(http.request.uri.path, \"/checkout\")"),
			Value:       cloudflare.F("script-src 'none';"),
