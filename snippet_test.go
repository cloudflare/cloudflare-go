...
func TestSnippetReplaceWithOptionalParams(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Snippets.Replace(
		context.TODO(),
		"023e105f4ecef8ad9ca31a8372d0c353",
		"snippet_name_01",
		cloudflare.SnippetReplaceParams{
			Files: cloudflare.F("export { async function fetch(request, env) {return new Response('some_response') } }"),
			Metadata: cloudflare.F(cloudflare.SnippetReplaceParamsMetadata{
