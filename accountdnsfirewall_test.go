...
func TestAccountDNSFirewallUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.Accounts.DNSFirewalls.Update(
		...
		cloudflare.AccountDNSFirewallUpdateParams{
			...
			OriginIPs:            cloudflare.F([]cloudflare.AccountDNSFirewallUpdateParamsOriginIP{shared.UnionString("192.0.2.1"), shared.UnionString("198.51.100.1"), shared.UnionString("2001:DB8:100::CF")}),
			AttackMitigation: cloudflare.F(cloudflare.AccountDNSFirewallUpdateParamsAttackMitigation{
				Enabled:                 cloudflare.F(true),
				OnlyWhenOriginUnhealthy: cloudflare.F(false),
			}),
...
func TestAccountDNSFirewallDNSFirewallNewDNSFirewallClusterWithOptionalParams(t *testing.T) {
	...
	_, err := client.Accounts.DNSFirewalls.DNSFirewallNewDNSFirewallCluster(
		...
		cloudflare.AccountDNSFirewallDNSFirewallNewDNSFirewallClusterParams{
			Name:      cloudflare.F("My Awesome DNS Firewall cluster"),
			OriginIPs: cloudflare.F([]cloudflare.AccountDNSFirewallDNSFirewallNewDNSFirewallClusterParamsOriginIP{shared.UnionString("192.0.2.1"), shared.UnionString("198.51.100.1"), shared.UnionString("2001:DB8:100::CF")}),
...
func TestAccountDNSFirewallDNSFirewallListDNSFirewallClusters(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Accounts.DNSFirewalls.DNSFirewallListDNSFirewallClusters(context.TODO(), "023e105f4ecef8ad9ca31a8372d0c353")
	if err != nil {
		var apierr *cloudflare.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
